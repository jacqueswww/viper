...
def optimize(node):
    ...
    if node.value in arith and int_at(argz, 0) and int_at(argz, 1):
        ...
        if argz[0].annotation and argz[1].annotation:
            ...
            annotation = ''
        return LLLnode(new_value, [], node.typ, None, node.pos, annotation, add_gas_estimate=node.add_gas_estimate, debug=node.debug)
        ...
        return LLLnode(argz[1].value, argz[1].args, node.typ, node.location, node.pos, argz[1].annotation, add_gas_estimate=node.add_gas_estimate, debug=node.debug)
    elif node.value == "add" and get_int_at(argz, 1) == 0:
        return LLLnode(argz[0].value, argz[0].args, node.typ, node.location, node.pos, argz[0].annotation, add_gas_estimate=node.add_gas_estimate, debug=node.debug)
        ...
        if get_int_at(argz, 0, True) > get_int_at(argz, 1, True):
            ...
            return LLLnode("clample", [argz[1], argz[2]], node.typ, node.location, node.pos, node.annotation, add_gas_estimate=node.add_gas_estimate, debug=node.debug)
    elif node.value == "clamp_nonzero" and int_at(argz, 0):
        if get_int_at(argz, 0) != 0:
            return LLLnode(argz[0].value, [], node.typ, node.location, node.pos, node.annotation, add_gas_estimate=node.add_gas_estimate, debug=node.debug)
        else:
            raise Exception("Clamp always fails")
    # Turns out this is actually not such a good optimization after all
        ...
        for arg in argz:
            if arg.value == "seq":
                ...
                o.append(arg)
        return LLLnode(node.value, o, node.typ, node.location, node.pos, node.annotation, add_gas_estimate=node.add_gas_estimate, debug=node.debug)
    elif hasattr(node, 'total_gas'):
        o = LLLnode(node.value, argz, node.typ, node.location, node.pos, node.annotation, add_gas_estimate=node.add_gas_estimate, debug=node.debug)
        o.total_gas = node.total_gas - node.gas + o.gas
        o.func_name = node.func_name
        return o
    else:
        return LLLnode(node.value, argz, node.typ, node.location, node.pos, node.annotation, add_gas_estimate=node.add_gas_estimate, debug=node.debug)
